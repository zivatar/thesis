...
urlpatterns = [
                  ...
                  url(r'^site/(?P<pk>[0-9]+)/climate/(?P<year>[0-9]+)/(?P<month>[0-9]+)$', views.climate,
                      name='climate'),
                  ...
                  url(r'^site/(?P<site>[0-9]+)/(?P<year>[0-9]+)/(?P<month>[0-9]+)$', views.monthly_view,
                      name='monthly_view'),
                  url(r'^site/(?P<site>[0-9]+)/delete_image/(?P<number>[0-9]+)$', views.delete_site_image,
                  ...
                  url(r'^accounts/password_reset/$', auth_views.password_reset,
                      {'template_name': 'registration/z_password_reset.html'}, name='password_reset'),
                  url(r'^accounts/password_reset/done/$', auth_views.password_reset_done,
                      {'template_name': 'registration/z_password_reset_done.html'}, name='password_reset_done'),
                  url(r'^accounts/reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
                      auth_views.password_reset_confirm,
                      {'template_name': 'registration/z_password_reset_confirm.html'}, name='password_reset_confirm'),
                  url(r'^accounts/password_reset/complete/$', auth_views.password_reset_complete,
                      {'template_name': 'registration/z_password_reset_complete.html'}, name='password_reset_complete'),
